...
{
  dataset="../experiments/exp4/renault_small_part1"
  output="~/code/experiments/exp4/renault_small_part1"
  
#  dataset="renault_small_header"
#  output=paste("~/code/experiments/exp4/", dataset, "/", sep='')
  header=TRUE
  sauv=TRUE
 ...
 #for(i in 0:0)
  { 
    for(j in 0:1)
    {
      ...
      {
        print(j)
        training_set = read.csv(file=paste(paste(paste(paste('~/code/datasets/', dataset, sep=''),'/set',sep=''),as.character(j),sep=''),'_exemples.csv',sep=''), header=header)
      }
      else if(i != j)
      {
        print(j)
        training_set = rbind(training_set, read.csv(file=paste(paste(paste(paste('~/code/datasets/', dataset, sep=''),'/set',sep=''),as.character(j),sep=''),'_exemples.csv',sep=''), header=header))
      }
    }
    ...
    fichier = paste(output,'/BN_', as.character(i),'.xml',sep='')
