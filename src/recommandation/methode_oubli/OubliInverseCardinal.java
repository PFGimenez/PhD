...
public class OubliInverseCardinal extends MethodeOubliRestauration {

	public OubliInverseCardinal(int seuil)
	{
		super(seuil, null);
	}
	...
	public Map<String, Double> recommandation(Var v, HashMap<String, String> historiqueOperations, VDD vdd, ArrayList<String> possibles, SALADD contraintes)
	{
		 ...
		 */
		if(historiqueOperations.size() <= 3)
		...
		for(int i = 0; i < size-2; i++)
		{
			...
			for(int j = i+1; j < size-1; j++)
			{
				...
				{
					String s3 = set[k];
					vdd.conditioner(vdd.getVar(s3), vdd.getVar(s3).conv(historiqueOperations.get(s3)));
					...
					vdd.deconditioner(vdd.getVar(s3));
				}
		...
		vdd.conditioner(vdd.getVar(m3), vdd.getVar(m3).conv(historiqueOperations.get(m3)));
