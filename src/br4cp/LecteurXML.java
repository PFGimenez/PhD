...
public void lectureBIF(String nomFichier) {
	...
	public void reorganiseContraintes(int heuristique){
		reorga=new ArrayList<Integer>();
		
		//0=rien
		if(heuristique==0){
			for(int i=0; i<getNbConstraints(); i++)
				reorga.add(i);
		}
		...
		if(heuristique==-2){
			for(int i=0; i<getNbConstraints(); i++)
				reorga.add(getNbConstraints()-(i+1));
		}
		...
		if(heuristique==-1){
			reorga.add(0);
			int random;
			for(int i=1; i<getNbConstraints(); i++){
		...
		if(heuristique==1){
			int max=0;
			int maxVal=-1;
			for(int j=0; j<getNbConstraints(); j++){
				for(int i=0; i<getNbConstraints(); i++){
		...
		//2=amilastre : v1 puis v1 v2 puis v1 v2 v3, ...
		if(heuristique==2){
		...
		//3=ecart max max		score
		if(heuristique==3){
			int valscore;
			int[] score=new int[getNbConstraints()];
			for(int i=0; i<getNbConstraints(); i++){
				int max=0;
				if(cons[i]!=null){
					for(int j=0; j<cons[i].scopeID.length; j++){
		...
		//5=domain max* domain max * ecart max
		if(heuristique==5){
			int ecart;
			int domain;
			int[] score=new int[getNbConstraints()];
		...
		//7=domain max* domain max * ecart max - hard first
		if(heuristique==7){
			int ecart;
			int domain;
			//int sumdomain;
		...
		//8= prod des domaines * ecart max + hard first
		if(heuristique==8){
			long ecart;
			long[] score=new long[getNbConstraints()];
			for(int i=0; i<getNbConstraints(); i++)
				score[i]=-1;
		...
		//9=dom*ecart+dom*dom*ecart+domdomdomecar+ ... - hard first
		if(heuristique==9){
			int proddomain=1;
			int curr=var.size();
			int next=var.size();
